...
comp <- s16 %>%  
  ...
  mutate(gender_match = gender_16 == gender_17,
         ...
         age_match = age_17 - age_16 <= 1 & age_17 - age_16 >= 0,
         race_match = race_recode_16 == race_recode_17) %>% 
  alphabetize_cols()

# How often do all 4 match?
comp %>% 
  select(contains(match = "match")) %>% 
  rowSums() %>% freq()

# example: not same person
...
# Graph categorical frequencies, 10 or less distinct values
graph_freqs(s16, cutoff = 10)

# Graph binaries that are in both datasets
